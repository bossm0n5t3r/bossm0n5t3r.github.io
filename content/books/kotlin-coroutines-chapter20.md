...
- 함수형이 많이 복잡해진 상태이므로, `emit` 이라는 추상 메서드를 가진 `FlowController` 함수형 인터페이스를 정의해 간단히 만들어 보자
...
- `it` 에서 `emit` 을 호출하는 것 또한 불편 → `FlowController` 를 **리시버**로 만들자
  - 이렇게 하면 람다식 내부에 `FlowController` 타입의 (`this` 로 지정 가능한) **리시버가 생김**
...
- 이제 됐다.
  - 위에서 플로우를 구현한 방식은 `Flow`, `FlowController`, `flow` 가 실제 구현한 방식과 거의 동일
